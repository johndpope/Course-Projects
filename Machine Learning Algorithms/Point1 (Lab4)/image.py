...
def read_image(filename):
	...
	image = open(filename)
	img_type = image.readline()
	(width, height) = [int(i) for i in image.readline().split()]
	max_value = int(image.readline())
	img = []
	...
	if img_type[:2] == "P5":
		for i in range(height):
			...
			for j in range(width):
				row.append(float(ord(image.read(1))))
			img.append(row)
		...
		for i in range(height):
			row = []
			for j in range(width):
				r = float(ord(image.read(1)))
				g = float(ord(image.read(1)))
				b = float(ord(image.read(1)))
				rgb = [r,g,b]
				row.append(rgb)
			img.append(row)
	image.close()
...
def label_image(img, cluster_centroids):
	...
	for i in range(height):
		row = []
		for j in range(width):
			temp = [distance(centroid, img[i][j]) for centroid in cluster_centroids]
...
def write_image(filename, img):
	...
	if type(img[0][0]) == list:
		...
		for i in range(height):
			for j in range(width):
				f.write(bytearray(img[i][j]))
	else:
		max_val = max(max(img))
		out = "P5\n"+str(width)+" "+str(height)+"\n"+str(max_val)+"\n"
		f.write(out) 
		for i in range(height):
			f.write(bytearray(img[i]))
	f.close()
...
def decompress_image(cluster_labels, cluster_centroids):
	...
	for i in range(height):
		row = []
		for j in range(width):
			pixel = list(cluster_centroids[int(cluster_labels[i][j])])
