...
class PerceptronClassifier:
    ...
    def train( self, trainingData, trainingLabels, sample_weights=None):
        ...
        for iteration in range(self.max_iterations):
            for i in range(len(trainingData)): 
                y_dash = np.dot(trainingData[i], self.weights)
                if y_dash*trainingLabels[i] <= 0:
    ...
    def sample_data(self, trainingData, trainingLabels, sample_weights):
        indices = util.nSample(sample_weights, range(len(sample_weights)), int(0.6*len(sample_weights)) );
        new_trainingData = [trainingData[i] for i in indices]
        new_trainingLabels = [trainingLabels[i] for i in indices]
        return new_trainingData, new_trainingLabels
